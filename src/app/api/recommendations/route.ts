...
export async function POST(req: Request) {
  try {
    ...
    if (themeError || !theme) {
      return NextResponse.json({ error: "Theme opportunity not found" }, { status: 404 });
    }
    ...
    const { data: subOpportunities } = await supabase
      .from("opportunities")
...

{
  "recommendations": [
    ...
    const response = await recommender.generate(prompt);
    ...
    try {
      ...
      console.error("Failed to parse recommender output:", parseError);
    ...
    // Upsert one row per tier. The unique index (opportunity_id, type) means
    // regenerating REPLACES the existing rec at that tier, which is what we
    // want — the human hasn't approved it yet, or they explicitly asked for
    // a fresh take.
